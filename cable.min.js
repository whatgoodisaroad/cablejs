...
value:obj.defaultValue,wireup:obj.wireup,isWiredUp:false,"in":[],out:[],scope:scope}},library:function(name,obj,scope){graph[name]={type:"library",path:obj.path,shim:obj.shim,handle:null,loaded:false,scope:scope}},sub:function(name,obj,scope){var newObj={};var references={};each(obj,function(subobj,subname){if(subobj.type==="reference")references[subname]=subobj.referenceName});each(obj,function(subobj,subname){var newName=subname==="main"?name:name+"_"+subname;newObj[newName]=subobj});Cable.define(newObj,
{reify:false,wireup:false,scope:{chain:scope.chain.concat([name])}})}};function enumerateScopes(chain,name){var suffix=name==="main"?"":"_"+name,scopes=[];for(var idx=0;idx<chain.length;++idx)scopes.push(chain.slice(0,chain.length-idx).join("_")+suffix);scopes.push(name);return scopes}function resolve(name,scope){var names=enumerateScopes(scope.chain,name);for(var idx=0;idx<names.length;++idx)if(graph.hasOwnProperty(names[idx]))return names[idx];return null}function reify(){each(graph,function(node){node.out=
[]});each(graph,function(node,nodeName){if(node.fn)getDependencies(node.fn,node.context).forEach(function(depName){var qname=resolve(depName,node.scope);if(qname)graph[qname].out.push(nodeName);else{function showContext(context){var refs=[];each(context.references,function(ref,name){refs.push("'"+name+"' ==> '"+ref+"'")});return"named '"+context.name+"'"+" and references {"+refs.join(", ")+"}"}throw"Reference to undefined node '"+depName+"' as dependency of '"+nodeName+"'"+(node.hasOwnProperty("context")?
" in context "+showContext(node.context):"");}})})}function wireup(){each(graph,function(node,nodeName){if(node.type==="event"&&!node.isWiredUp){node.wireup(function(value){yield(nodeName,function(setter){setter(value)})});node.isWiredUp=true}})}var yields={data:function(name,fn){var access=function(){if(!arguments.length)return graph[name].value;else if(arguments[0]!=graph[name].value){graph[name].value=arguments[0];triggerDownstream(name)}};each(graph[name].helpers,function(helper,helpName){access[helpName]=
function(){return helper.apply(access,arguments)}});fn(access)},synthetic:function(name,fn){fn(function(){return graph[name].value})},effect:function(){},event:function(name,fn){fn(function(){if(!arguments.length)return graph[name].value;else if(arguments[0]!=graph[name].value||!graph[name].coalesce){graph[name].value=arguments[0];triggerDownstream(name)}})},library:function(name,fn){if(graph[name].loaded)fn(graph[name].handle);else evaluate(name,function(){yield(name,fn)})}};function yield(name,
fn){if(graph.hasOwnProperty(name))yields[graph[name].type](name,fn);else throw"Cannot yield: '"+name+"' is not defined";}Cable.yield=yield;function yieldAll(names,fn,prefix){if(prefix===undefined)prefix=[];if(!names.length)fn(prefix);else yield(names[0],function(value){yieldAll(names.slice(1),fn,prefix.concat([value]))})}function yieldIn(name,fn){var resolved=graph[name]["in"].map(function(dep){return resolve(dep,graph[name].scope)});yieldAll(resolved,fn)}function evaluate(name,fn){evaluators[graph[name].type](name,
fn)}var evaluators={data:yield.data,event:yield.event,synthetic:function(name,fn){yieldIn(name,function(deps){deps.splice(graph[name].resultIndex,0,function(result){if(graph[name].value!=result||!graph[name].coalesce){graph[name].value=result;graph[name].invoked=true;triggerDownstream(name)}fn()});graph[name].fn.apply({},deps)})},effect:function(name,fn){yieldIn(name,function(deps){graph[name].fn.apply({},deps);fn()})},library:function(name,fn){if(graph[name].loaded){fn();return}var define=function(){var idx=
arguments.length-1;if(isFunction(arguments[idx]))graph[name].handle=arguments[idx]();else graph[name].handle=arguments[idx];graph[name].loaded=true};define.amd={};var req=new XMLHttpRequest;req.onreadystatechange=function(){if(this.readyState===4&&this.status===200){var source=this.responseText;if(graph[name].shim)source=["define(function() {",source,"\nreturn ",graph[name].shim,"; });"].join("");window.define=define;eval(source);delete window.define;if(name==="$"&&($&&$.noConflict))$.noConflict();
fn()}};req.open("get",graph[name].path,true);req.send()}};function trigger(name){evaluate(name,function(){})}function triggerDownstream(name){graph[name].out.forEach(trigger)}})();Cable.data=function(value,helpers){var obj={type:"data",value:value};if(helpers)obj.helpers=helpers;return obj};
Cable.event=function(selector,events,property,triggerOnLoad){return{type:"event",coalesce:false,wireup:function(fn){Cable.yield("$",function($){if(selector==="document"&&events==="ready")$(document).ready(fn);else{var getter=function(){var val=null,obj=$(selector);if(!property)property="time";if(property==="value")val=obj.val();else if(property==="time")val=new Date;else if(/^data-[-_a-zA-Z0-9]+/.test(property))val=obj.attr(property);fn(val)},handler;if(events==="key-return"){events="keyup";handler=
function(evt){if(evt.keyCode===13)getter()}}else handler=getter;$(document).on(events,selector,handler);if(triggerOnLoad)getter()}})}}};Cable.returnKey=function(selector){return{type:"event",wireup:function(fn){Cable.yield("$",function($){$(selector).on("keyup",function(evt){if(evt.keyCode===13)fn(new Date)})})}}};Cable.define({init:{type:"event",wireup:function(f){f(new Date)}}},{reify:false,wireup:false});
...
Cable.library=function(path,shim){return{type:"library",path:path,shim:shim}};Cable.template=function(selector,template){var reg=/\{\{[_a-zA-Z0-9]+\}\}/g,match=template.match(reg)||[],deps=match.map(function(m){return m.replace(/\{|\}/g,"")});var obj={properties:Cable.pack(deps),main:function(properties,$){var rend=template;for(var idx=0;idx<deps.length;++idx)rend=rend.replace("{{"+deps[idx]+"}}",properties()[deps[idx]]);$(selector).html(rend)}};return obj};
