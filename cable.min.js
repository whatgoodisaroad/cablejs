...
var Cable={};
(function(){var reserved="result respond define type event".split(" ");var graph={};function each(obj,fn){for(var key in obj)if(obj.hasOwnProperty(key))fn(obj[key],key)}function getArgNames(fn){if(fn.argAliases)return fn.argAliases;else return(fn+"").match(/^function(\s*)?\(([^)]*)\)/m)[2].split(",").map(function(x){return x.replace(/(^\s+)|(\s+$)/g,"")})}function getFanIn(fn,context){return getArgNames(fn).filter(function(arg){return reserved.indexOf(arg)==-1}).map(function(arg){return arg.replace(/^_/,"")})}
...
function(helper,helpName){access[helpName]=function(){return helper.apply(access,arguments)}});fn(access)},synthetic:function(name,fn){if(!graph[name].invoked)evaluate(name,function(){});else fn(function(){return graph[name].value})},effect:function(){},event:function(name,fn){fn(function(){if(!arguments.length)return graph[name].value;else if(arguments[0]!=graph[name].value||!graph[name].coalesce){graph[name].invoked=true;graph[name].value=arguments[0];triggerDownstream(name)}})},library:function(name,
fn){if(graph[name].loaded)fn(graph[name].handle);else evaluate(name,function(){yield(name,fn)})}};function yield(name,fn){if(graph.hasOwnProperty(name))yields[graph[name].type](name,fn);else throw"Cannot yield: '"+name+"' is not defined";}Cable.yield=yield;function yieldAll(names,fn,prefix){if(prefix===undefined)prefix=[];if(!names.length)fn(prefix);else yield(names[0],function(value){yieldAll(names.slice(1),fn,prefix.concat([value]))})}function yieldIn(name,fn){var resolved=graph[name]["in"].map(function(dep){return resolve(dep,
graph[name].scope)});yieldAll(resolved,fn)}function evaluate(name,fn){evaluators[graph[name].type](name,fn)}var evaluators={data:yield.data,event:yield.event,synthetic:function(name,fn){yieldIn(name,function(deps){deps.splice(graph[name].resultIndex,0,function(result){graph[name].invoked=true;if(graph[name].value!=result||!graph[name].coalesce){graph[name].value=result;triggerDownstream(name)}fn()});graph[name].fn.apply({},deps)})},effect:function(name,fn){yieldIn(name,function(deps){graph[name].fn.apply({},
deps);fn()})},library:function(name,fn){if(graph[name].loaded){fn();return}var define=function(){var idx=arguments.length-1;if(isFunction(arguments[idx]))graph[name].handle=arguments[idx]();else graph[name].handle=arguments[idx];graph[name].loaded=true};define.amd={};var req=new XMLHttpRequest;req.onreadystatechange=function(){if(this.readyState===4&&this.status===200){var source=this.responseText;if(graph[name].shim)source=["define(function() {",source,"\nreturn ",graph[name].shim,"; });"].join("");
window.define=define;eval(source);delete window.define;if(name==="$"&&($&&$.noConflict))$.noConflict();fn()}};req.open("get",graph[name].path,true);req.send()}};function trigger(name){if(!allDependenciesEvaluated(name))return;evaluate(name,function(){})}function allDependenciesEvaluated(name){var deps=graph[name]["in"].map(function(dep){return resolve(dep,graph[name].scope)});for(var idx=0;idx<deps.length;++idx)if(graph[deps[idx]].type==="event"&&!graph[deps[idx]].invoked)return false;return true}
function triggerDownstream(name){graph[name].out.forEach(trigger)}Cable.initialize=function(name,value){if(graph[name].type==="synthetic"){graph[name].value=value;graph[name].invoked=true}};Cable._debug=function(){return graph}})();Cable.withArgs=function(args,fn){fn.argAliases=args;return fn};Cable.data=function(value,helpers){var obj={type:"data",value:value};if(helpers)obj.helpers=helpers;return obj};
Cable.event=function(selector,events,property,triggerOnLoad){return{type:"event",coalesce:false,wireup:function(fn){Cable.yield("$",function($){if(selector==="document"&&events==="ready")$(document).ready(fn);else{var getter=function(){var val=null,obj=$(selector);if(!property)property="time";if(property==="value"){val=obj.val();if(obj.is("[type='number']"))val=parseFloat(val)}else if(property==="time")val=new Date;else if(/^data-[-_a-zA-Z0-9]+/.test(property))val=obj.attr(property);else if(property===
":checked")val=obj.is(":checked");fn(val)},handler;if(events==="key-return"){events="keyup";handler=function(evt){if(evt.keyCode===13)getter()}}else handler=getter;$(document).on(events,selector,handler);if(triggerOnLoad)getter()}})}}};Cable.textbox=function(selector){return Cable.event(selector,"change keyup","value",true)};Cable.checkbox=function(selector){return Cable.event(selector,"change",":checked",true)};
Cable.returnKey=function(selector){return{type:"event",wireup:function(fn){Cable.yield("$",function($){$(selector).on("keyup",function(evt){if(evt.keyCode===13)fn(new Date)})})}}};Cable.define({init:{type:"event",wireup:function(f){f(new Date)}}},{reify:false,wireup:false});
...
Cable.json=function(fn){return{url:fn,main:function($,url,result){var cdr=/^http/.test(url())&&!/callback=/.test(url());$.ajax({dataType:"json",url:url(),crossDomain:!!cdr,success:function(data){result(data)}})}}};Cable.text=function(url){return function($,result){$.ajax({url:url,dataType:"text",success:function(text){result(text)}})}};
