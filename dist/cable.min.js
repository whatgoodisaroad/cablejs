...
Cable.button=function(c){return Cable.withArgs(["$","define"],function(b,g){g(Cable.withArgs(["event"],function(g){b(c).on("click",function(){g(new Date)})}))})};Cable.returnKey=function(c){return Cable.withArgs(["$","define"],function(b,g){g({type:"event",wireup:function(g){b(c).on("keyup",function(b){13===b.keyCode&&g(new Date)})}})})};
